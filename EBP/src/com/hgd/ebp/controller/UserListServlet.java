...
public class UserListServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		UserSearchState state=null;
		if(null==page)
		{
			page="0";
			session.removeAttribute("UserSearchState");
			state=new UserSearchState();
		}
		else
		{
			state=(UserSearchState)session.getAttribute("UserSearchState");
			if(null==state)
				state=new UserSearchState();
		}
		...
		try {
			int lastPage=userService.getLastPage(state);
			state.setLastPage(lastPage);
			List<User> userList=userService.SearchUserByPage(state, page);
			session.setAttribute("UserSearchState", state);
